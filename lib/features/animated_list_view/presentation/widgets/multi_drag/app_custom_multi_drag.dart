...
/// Signature for when [AppCustomMultiDragGestureRecognizer] recognizes the start of a drag gesture.
typedef GestureMultiDragStartCallback = Drag? Function(Offset position);

/// Per-pointer state for a [AppCustomMultiDragGestureRecognizer].
///
/// A [AppCustomMultiDragGestureRecognizer] tracks each pointer separately. The state for
/// each pointer is a subclass of [AppCustomMultiDragPointerState].
abstract class AppCustomMultiDragPointerState {
  /// Creates per-pointer state for a [AppCustomMultiDragGestureRecognizer].
  ///
  /// The [initialPosition] argument must not be null.
  AppCustomMultiDragPointerState(
      this.initialPosition, this.kind, this.gestureSettings)
...
///
/// In contrast to [DragGestureRecognizer], [AppCustomMultiDragGestureRecognizer] watches
...
///
/// [AppCustomMultiDragGestureRecognizer] is not intended to be used directly. Instead,
/// consider using one of its subclasses to recognize specific types for drag
/// gestures.
///
/// See also:
///
///  * [AppCustomImmediateMultiDragGestureRecognizer], the most straight-forward variant
...
///    start vertically.
///  * [AppCustomDelayedMultiDragGestureRecognizer], which only recognizes drags that
///    start after a long-press gesture.
abstract class AppCustomMultiDragGestureRecognizer extends GestureRecognizer {
  /// Initialize the object.
  ///
  /// {@macro flutter.gestures.GestureRecognizer.supportedDevices}
  AppCustomMultiDragGestureRecognizer({
  ...
  Map<int, AppCustomMultiDragPointerState>? _pointers =
      <int, AppCustomMultiDragPointerState>{};

  @override
  void addAllowedPointer(PointerDownEvent event) {
    assert(_pointers != null);
    assert(!_pointers!.containsKey(event.pointer));
    final AppCustomMultiDragPointerState state = createNewPointerState(event);
  ...
  @protected
  @factory
  AppCustomMultiDragPointerState createNewPointerState(PointerDownEvent event);

  void _handleEvent(PointerEvent event) {
    assert(_pointers != null);
    assert(_pointers!.containsKey(event.pointer));
    final AppCustomMultiDragPointerState state = _pointers![event.pointer]!;
    coloredPrint('///////////////////////');
    coloredPrint('_handle event.viewId', event.viewId);
    coloredPrint('_handle event.embedderId', event.embedderId);
    coloredPrint('_handle event.timeStamp', event.timeStamp);
    coloredPrint('_handle event.pointer', event.pointer);
    coloredPrint('_handle event.kind', event.kind);
    coloredPrint('_handle event.device', event.device);
    coloredPrint('_handle event.position', event.position);
    coloredPrint('_handle event.delta', event.delta);
    coloredPrint('_handle event.buttons', event.buttons);
    coloredPrint('_handle event.down', event.down);
    coloredPrint('_handle event.obscured', event.obscured);
    coloredPrint('_handle event.pressure', event.pressure);
    coloredPrint('_handle event.pressureMin', event.pressureMin);
    coloredPrint('_handle event.pressureMax', event.pressureMax);
    coloredPrint('_handle event.distance', event.distance);
    coloredPrint('_handle event.distanceMax', event.distanceMax);
    coloredPrint('_handle event.size', event.size);
    coloredPrint('_handle event.radiusMajor', event.radiusMajor);
    coloredPrint('_handle event.radiusMinor', event.radiusMinor);
    coloredPrint('_handle event.radiusMin', event.radiusMin);
    coloredPrint('_handle event.radiusMax', event.radiusMax);
    coloredPrint('_handle event.orientation', event.orientation);
    coloredPrint('_handle event.tilt', event.tilt);
    coloredPrint('_handle event.platformData', event.platformData);
    coloredPrint('_handle event.synthesized', event.synthesized);
    coloredPrint('_handle event.transform', event.transform);
    coloredPrint('_handle event.original', event.original);
    coloredPrint('============================================');
  ...
  void acceptGesture(int pointer) {
    assert(_pointers != null);
    final AppCustomMultiDragPointerState? state = _pointers![pointer];
    if (state == null) {
...
class _ImmediatePointerState extends AppCustomMultiDragPointerState {
...
///
/// In contrast to [PanGestureRecognizer], [AppCustomImmediateMultiDragGestureRecognizer]
...
///  * [AppCustomDelayedMultiDragGestureRecognizer], which only recognizes drags that
///    start after a long-press gesture.
class AppCustomImmediateMultiDragGestureRecognizer
    extends AppCustomMultiDragGestureRecognizer {
  /// Create a gesture recognizer for tracking multiple pointers at once.
  ///
  /// {@macro flutter.gestures.GestureRecognizer.supportedDevices}
  AppCustomImmediateMultiDragGestureRecognizer({
    super.debugOwner,
    super.supportedDevices,
...
class _HorizontalPointerState extends AppCustomMultiDragPointerState {
...
///  * [AppCustomImmediateMultiDragGestureRecognizer], a similar recognizer, but without
...
class _VerticalPointerState extends AppCustomMultiDragPointerState {
...
class _DelayedPointerState extends AppCustomMultiDragPointerState {
...
/// after a delay.
///
/// In contrast to [AppCustomImmediateMultiDragGestureRecognizer],
/// [AppCustomDelayedMultiDragGestureRecognizer] waits for a [delay] before recognizing
/// the drag. If the pointer moves more than [kTouchSlop] before the delay
/// expires, the gesture is not recognized.
///
/// In contrast to [PanGestureRecognizer], [AppCustomDelayedMultiDragGestureRecognizer]
/// watches each pointer separately, which means multiple drags can be
/// recognized concurrently if multiple pointers are in contact with the screen.
///
/// See also:
...
///  * [PanGestureRecognizer], which recognizes only one drag gesture at a time,
///    regardless of how many fingers are involved.
class AppCustomDelayedMultiDragGestureRecognizer
    extends AppCustomMultiDragGestureRecognizer {
  ...
  ///
  /// {@macro flutter.gestures.GestureRecognizer.supportedDevices}
  AppCustomDelayedMultiDragGestureRecognizer({
